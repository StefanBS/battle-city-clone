...
# Initialize Pygame locally for event creation if needed, but InputHandler itself doesn't require it
# pygame.init() # Uncomment if tests require full pygame init
...
def test_key_hold_and_release(handler: InputHandler, key_down_event, key_up_event) -> None:
...
def test_ignore_unmapped_keys(handler: InputHandler, key_down_event, key_up_event) -> None:
...
def test_repeated_keydown(handler: InputHandler, key_down_event) -> None:
    """Test that repeated KEYDOWN events for the same key don't change state after the first."""
    event = key_down_event(pygame.K_UP)
    handler.handle_event(event)
    assert handler.directions["up"]
...
def test_repeated_keyup(handler: InputHandler, key_down_event, key_up_event) -> None:
    """Test that repeated KEYUP events for the same key don't change state after the first."""
